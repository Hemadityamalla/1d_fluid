...
program fluid1D
	...
	!Initializing the parameters used for the simulation
        double precision :: x0=0.0, xL=512.0, dx=0.1, tFinal=200.0, dt = 0.01
        double precision :: D=0.1, Eb=-1.0, xb=31.0, seedAmplitude = 0.01
	...
	real, dimension(20000) :: tStep, fPos !Array to collect the position of the front
        ...
        !Scales used to convert all my units to dimensional units (kg, cm, sec)
        double precision, parameter :: l0 = 2.3e-4, t0 = 3e-12, n0 = 4.7e14, D0 = 1.8e4, E0 = 2e5

        double precision, parameter :: mu = 380.0, alpha0 = 4332.0, epsilon0 = 8.85418782e-14, chargeE = 1.60217662e-19
        !Converting all my parameters to dimensional units
        x0 = x0*l0
        xL = xL*l0
        dx = dx*l0
        tFinal = tFinal*t0
        ...
        Eb = Eb*E0
        xb = xb*l0
        seedAmplitude = seedAmplitude*n0
	...
	!Initial conditions
	ne(3:N+2) = seedAmplitude*exp(-(x - xb)**2)
	np(3:N+2) = seedAmplitude*exp(-(x - xb)**2)
	E_CF(N+1) = Eb
	call calc_electricField(N,nGhost, dx, ne, np, E, E_CF, epsilon0, chargeE) !Calculating the electric field at the cell centers
	!Ghost cells
	ne(1) = -ne(4)
	ne(2) = -ne(3)
	np(1) = -np(4)
	...
	do while (time < tFinal)
		...
		call calc_source(N,nGhost, ne, E, s, E0)
		call calc_diffusionFlux(N,nGhost, ne, E, dx, D, df)

		!Upate solutions for t_n/2
		neNew(3:N+2) = ne(3:N+2) + dt*(mu*af + df + mu*alpha0*s)
		npNew(3:N+2) = np(3:N+2) + dt*(mu*alpha0*s)
		call calc_electricField(N,nGhost, dx, neNew, npNew, ENew, E_CF, epsilon0, chargeE)
		!Ghost cells
		neNew(1) = -neNew(4)
		neNew(2) = -neNew(3)
		npNew(1) = -npNew(4)
		...
		call calc_source(N,nGhost, neNew, ENew, snew, E0)
		call calc_diffusionFlux(N,nGhost, neNew, ENew, dx, D, dfnew)

		!Update solution for t_n+1
		ne(3:N+2) = ne(3:N+2) + 0.5*dt*(mu*afnew + dfnew + mu*af + df + mu*alpha0*snew + mu*alpha0*s)
		np(3:N+2) = np(3:N+2) + 0.5*dt*mu*alpha0*(snew + s)
		call calc_electricField(N,nGhost, dx, ne, np, E, E_CF, epsilon0, chargeE)
		!Ghost cells
		ne(1) = -ne(4)
		ne(2) = -ne(3)
		np(1) = -np(4)
		...
		!if (sum(ne/n0) .gt. 1e10) stop 'Solution Diverging!'
                do i=1,N
		...
		iter = iter + 1
                ...
                if (mod(iter,100) .le. n0*1e-15) then
                tempVar1 = tempVar2
...
!Subroutines and functions used-------------------------------------------------------------------------------------------------------------------------
	...
	subroutine calc_source(N,Ng, ne, E, S, E0)
		implicit none
		integer, intent(in) :: N, Ng
		...
		S = ne(3:N+2)*abs(E(3:N+2))*exp(-E0/abs(E(3:N+2)))
	...
	subroutine calc_electricField(N,Ng, dx, ne, np, ECC, ECF, eps, cE)
		implicit none
		integer, intent(in) :: N, Ng
                double precision, intent(in) :: eps, cE
		...
		integer :: i
		do i=1,N
			ECF(N-i+1) = ECF(N-i+2) + (cE/eps)*dx*(ne(N-i+3) - np(N-i+3))
		end do
